...
# Partition stuff - Should work for x86 and $basearch
#zerombr
...
volgroup circle pv.01
logvol / --vgname=circle --size=8000 --name=root --grow --mkfsoptions "-m bigtime=0,inobtcount=0"
shutdown
...
sed -i 's/name: cloud-user/name: circle/g' /etc/cloud/cloud.cfg
echo -e 'circle\tALL=(ALL)\tNOPASSWD: ALL' >> /etc/sudoers
...
# XXX instance type markers - MUST match Circle Infra expectation
