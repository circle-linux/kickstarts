...
This branch has Circle Linux 9 specific kickstarts. These kickstarts vary
...
user can pick up, use, or modify to their liking to make their own Circle
...
% mock -r circle-9-x86_64 --init
% mock -r circle-9-x86_64 --install lorax-lmc-novirt vim-minimal pykickstart git
# You may need to be in permissive mode temporarily if you have issues
% setenforce 0
# Enter the shell
% mock -r circle-9-x86_64 --shell --isolation=simple --enable-network

# Clone the kickstarts and run an installation
% git clone https://github.com/circle-linux/kickstarts -b r9
% cd kickstarts
% livemedia-creator --ks Circle-9-Workstation.ks \
  --no-virt \
  --resultdir /var/lmc \
  --project="Circle Linux" \
  --make-iso \
  --volid Circle-Workstation-9 \
  --iso-only \
  --iso-name Circle-Workstation-9-x86_64.iso \
  --releasever=9 \
...
`/var/lib/mock/circle-9-x86_64/root/var/lmc`.
